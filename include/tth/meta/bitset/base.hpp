...
{
  private:
    inline int32_t Read_(Stream &stream) { return stream.Read(mWords, sizeof(mWords)); }
    inline int32_t Write_(Stream &stream) const { return stream.Write(mWords, sizeof(mWords)); }
